...
from currency_exchange.core.storage.base import BaseStorage
...
class SQLiteStorage(BaseStorage):
    ...
    def _store_new_providers(self, providers: object) -> object:
        conn = self.connection
        cursor = conn.cursor()
    ...
    def _store_new_projects(self, projects: object) -> object:
        conn = self.connection
        cursor = conn.cursor()

        read_sql = 'select code from projects where code in ({})'.\
            format(', '.join('?' * len(projects)))
        cursor.execute(read_sql, projects)
        in_db_projects = cursor.fetchall()
        in_db_projects = [p[0] for p in in_db_projects]

        diff = set(projects).difference(set(in_db_projects))
        new_projects = filter(lambda p: p in diff, set(projects))
        new_projects = map(lambda p: (p, p), new_projects)

        if new_projects:
            insert_sql = 'insert into projects (code, name) values (?, ?)'
            cursor.executemany(insert_sql, new_projects)
            conn.commit()
    ...
    def store_rates(self, exchange_rates):
        ...
        insert_sql = 'insert into currency_exchange_rate (from_currency, to_currency, on_date, provider, rate) values (?, ?, ?, ?, ?)'
    ...
    def get_currencies(self, provider_names, currency_codes):
        ...
        read_sql = 'select * from currency where code in ({})'.format(', '.join('?' * len(currency_codes)))
    ...
    def get_rates(self, provider_names, currency_code, to_currencies=None, on_date=None):
        ...
        filter_sql = 'provider=? and from_currency=?'
        ...
        query_sql = 'select * from currency_exchange_rate where {}'.format(filter_sql)
        ...
        for rec in data:
            tmp = CurrencyExchangeRate(
                ...
                currecies.get(rec[0]),
                currecies.get(rec[1]),
                decimal.Decimal(rec[4]),
                datetime.datetime.strptime(rec[2], '%Y-%m-%d')
            )
            result.append(tmp)
